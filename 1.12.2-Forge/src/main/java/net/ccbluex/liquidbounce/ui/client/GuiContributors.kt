...
class GuiContributors(private val prevGui: IGuiScreen) : WrappedGuiScreen() {
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        RenderUtils.drawRect(
            representedScreen.width / 4.0f,
            40.0f,
            representedScreen.width.toFloat(),
            representedScreen.height - 40.0f,
            Integer.MIN_VALUE
        )
        ...
        if (credits.isEmpty()) {
            if (failed) {
                ...
                Fonts.font40.drawCenteredString(
                    "Failed to load",
                    representedScreen.width / 8.0f,
                    representedScreen.height / 2.0f,
                    Color(255, gb, gb).rgb
                )
            } else {
                Fonts.font40.drawCenteredString(
                ...
                RenderUtils.drawLoadingCircle(
                    (representedScreen.width / 8).toFloat(),
                    (representedScreen.height / 2 - 40).toFloat()
                )
            }
        }
    ...
    private fun loadCredits() {
        try {
            ...
            for (credit in credits) {
                try {
                    HttpUtils.requestStream(
                        "${credit.avatarUrl}?s=${representedScreen.fontRendererObj.fontHeight * 4}",
                        "GET"
                    )?.use {
