...
class Projectiles : Module() {
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        if (classProvider.isItemBow(item)) {
            ...
            if (!classProvider.isItemSnowball(item) && !classProvider.isItemEnderPearl(item) && !classProvider.isItemEgg(
                    item
                )
            )
                return
        ...
        while (!hasLanded && posY > 0.0) {
            ...
            landingPosition = theWorld.rayTraceBlocks(
                posBefore, posAfter, stopOnLiquid = false,
            ...
            if (landingPosition != null) {
                hasLanded = true
                posAfter =
                    WVec3(landingPosition.hitVec.xCoord, landingPosition.hitVec.yCoord, landingPosition.hitVec.zCoord)
            }
