...
class LiquidWalk : Module() {
    ...
    fun onUpdate(event: UpdateEvent?) {
        ...
        when (modeValue.get().toLowerCase()) {
            "ncp", "vanilla" -> if (collideBlock(
                ...
                ) && thePlayer.isInsideOfMaterial(classProvider.getMaterialEnum(MaterialType.AIR)) && !thePlayer.sneaking
    ...
    fun onBlockBB(event: BlockBBEvent) {
        ...
        if (classProvider.isBlockLiquid(event.block) && !collideBlock(
                mc.thePlayer!!.entityBoundingBox,
                classProvider::isBlockLiquid
            ) && !mc.thePlayer!!.sneaking
        ) {
            when (modeValue.get().toLowerCase()) {
                "ncp", "vanilla" -> event.boundingBox = classProvider.createAxisAlignedBB(
                    event.x.toDouble(),
                    event.y.toDouble(),
                    event.z.toDouble(),
                    event.x + 1.toDouble(),
